...
<!-- Dans $reponseCart, on a la commande en cours (si elle existe) de l'utilisateur. -->
		...
		<?php } else {?>
			...
			<tbody>
				...
				<tr>
					<td >
						<a href="index.php?page=product&product=<?php echo $donneeCart['product_id']?>">
		...
		<div class="check">
			...
			<?php if (!$orderDisplay) {?>
				...
				<input type="hidden" name="page" value="deleteCart">
				<input type="submit" name="Cancel" value="Cancel" class="bouton">
			<?php } ?>
			</td>
			<td>
			<?php if (!$orderDisplay) {?>
